...
// Helper to invert values (using symmetry rule: Hole EI = -Shaft es; Hole ES = -Shaft ei)
const invert = (arr: number[]) => arr.map(v => -v);
...
const HOLE_DEVIATIONS: Record<string, number[]> = {
  ...
  'K': invert(SHAFT_DEVIATIONS['k']), // ES (K is usually transition/interference like shaft k)
  'M': invert(SHAFT_DEVIATIONS['m']), // ES
  'S': invert(SHAFT_DEVIATIONS['s'])  // ES
};

// --- EXPORTS FOR UI DROPDOWNS ---
export const SUPPORTED_IT_GRADES = ['5', '6', '7', '8', '9', '10', '11'];
export const SUPPORTED_HOLE_LETTERS = ['D', 'E', 'F', 'G', 'H', 'K', 'M', 'N', 'P', 'S'];
...
const TERMS = {
  ...
  fr: {
    ...
    gap: "Toujours un jeu",
    tight: "Toujours un serrage",
    mixed: "Jeu ou serrage possible",
    max_gap: "Jeu max",
    min_gap: "Jeu min",
    max_int: "Serrage max",
    min_int: "Serrage min",
    max_clearance: "Jeu max",
    default_rec: "Zone de tolérance ISO 286 standard."
  },
  ar: {
    ...
    interference: "تداخل",
    transition: "انتقالي",
    gap: "يوجد خلوص دائماً",
    tight: "يوجد تداخل دائماً",
