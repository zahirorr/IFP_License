...
import { BarChart3, Box, ChevronsLeftRight } from 'lucide-react';
...
const ToleranceVisualizer: React.FC<Props> = ({ data }) => {
  ...
  // --- CHART VIEW LOGIC ---
  const deviations = [hole.es, hole.ei, 0];
  if (shaft) {
    deviations.push(shaft.es, shaft.ei);
  }
  ...
  // --- ASSEMBLY VIEW LOGIC ---
  // We need to simulate the "actual" microns deviation based on the slider.
  // Tightest Fit (0%): Smallest Hole (EI) vs Largest Shaft (es)
  // Loosest Fit (100%): Largest Hole (ES) vs Smallest Shaft (ei)
  
  const currentHoleDev = hole.ei + (hole.es - hole.ei) * (simPercent / 100);
  
  // If no shaft, we just show hole scaling. If shaft exists, we scale it inversely for fit logic
  const currentShaftDev = shaft 
    ? shaft.es - (shaft.es - shaft.ei) * (simPercent / 100)
    : 0;

  const currentGap = currentHoleDev - currentShaftDev;
  
  // Visualization Scale (Exaggerated)
  // We want the max possible visible gap/interference to take up ~40% of the screen height
  const maxPossibleDiff = Math.max(
    Math.abs((shaft?.es || 0) - hole.ei), // Max Interference potential
    Math.abs(hole.es - (shaft?.ei || 0)), // Max Clearance potential
    Math.abs(hole.es), // Single mode fallback
    Math.abs(hole.ei)
  );
  
  const scaleFactor = maxPossibleDiff === 0 ? 1 : (80 / maxPossibleDiff); // 80px max deviation

  const centerY = 150;
  // Hole Top/Bottom are fixed "walls", we move the "surface"
  // Let's say Nominal is at +/- 60px from center.
  const nominalRadius = 60; 
  
  // Coordinate calculations
  // Hole Surface Y (Top) = -Nominal - Deviation
  // SVG Y is positive down. Center is 150.
  // Top Nominal Line: 150 - 60 = 90.
  // Bottom Nominal Line: 150 + 60 = 210.
  
  // We exaggerate deviation by subtracting it from the radius (moving towards center for negative dev, away for positive)
  // Actually: Hole Radius = Nominal + Dev. 
  // Visual Radius = NominalPx + (Dev * Scale)
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col transition-colors duration-200">
      ...
      <div className="p-6 flex-1 flex flex-col items-center justify-center min-h-[300px] relative">
        ...
        {viewMode === 'chart' ? (
                ...
                <g transform="translate(60, 0)">
                    ...
                    {shaft && (
                        <>
                            ...
                            />
                            {/* Shaft Text */}
                            <text x={200} y={mapY(shaft.es) - 8} textAnchor="middle" className="fill-orange-600 dark:fill-orange-400 font-bold">{shaft.es > 0 ? '+' : ''}{shaft.es}</text>
                            <text x={200} y={mapY(shaft.ei) + 16} textAnchor="middle" className="fill-orange-600 dark:fill-orange-400 font-bold">{shaft.ei > 0 ? '+' : ''}{shaft.ei}</text>
                        </>
                    )}
                </g>
             </svg>
          ...
          <div className="w-full flex flex-col items-center">
                    ...
                    {shaft && (
                        <g>
                            ...
                            <g transform="translate(270, 0)">
                               {/* Only show dimension lines if there is a visible gap or simple view */}
                            </g>
                        </g>
                    )}

                    {/* Interference/Gap Indicator */}
                    {shaft && (
                        <g transform={`translate(150, ${centerY - nominalRadius})`}>
                            {/* We draw a marker at the interface */}
                            {/* A circle magnifying glass effect? Maybe too complex for now. */}
                            {/* Just text label */}
                        </g>
                    )}
                </svg>
            ...
            {shaft && (
                <div className="w-full max-w-xs bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">
                        <span>Tightest Fit</span>
                        <span>Loosest Fit</span>
                    </div>
                    <input 
                        type="range" 
                        min="0" 
            ...
            {!shaft && (
                <div className="text-sm text-slate-400 italic">Select a shaft grade to enable assembly simulation.</div>
            )}
          </div>
        )}
      ...
      {/* Legend Footer */}
      <div className="bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 px-6 py-3 flex gap-6 text-xs justify-center">
